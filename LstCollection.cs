...
namespace Jewelry
{
    ...
    {
        private List<Jewelry> lst;

        public LstCollection() => this.lst = new List<Jewelry>();

        public LstCollection(List<Jewelry> lst) => this.lst = new List<Jewelry>(lst);

        public override string ToString()
        {
            string res = "";
            foreach (Jewelry element in lst)
        ...
        public Jewelry this[int i]
        {
            get => lst[i];
            set => lst[i] = value;
        }

        public LstCollection AddItem(Jewelry item)
        {
            lst.Add(item);
            return this;
        }
        ...
        public List<Jewelry> Search(string subStr)
        {
            bool SearchPredicater(Jewelry e)
            {
        ...
        public LstCollection ReadJsonFile(string fileName)
        {
            using (StreamReader r = new StreamReader(fileName))
            {
                ...
                foreach (var child in dictionarys)
                {
                    try
                    {
                        Jewelry p = JsonConvert.DeserializeObject<Jewelry>(JsonConvert.SerializeObject( child ));
                        if (string.IsNullOrEmpty(p.Id)) p.Id = Guid.NewGuid().ToString();
                        this.AddItem(p);
                    }
